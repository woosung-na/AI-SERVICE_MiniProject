...
User Query
    ...
    ▼
┌─────────────┐
│  Supervisor │  ← 상태 기반 결정론적 라우터 (iteration ≤ 3)
└──────┬──────┘
       │
       ├──[1]──→ RAG Agent ──────→ FAISS + BM25 Ensemble
       │                           (이중 쿼리: 긍정 k=5 + 반론 k=3)
       │
       ├──[2]──→ Web Agent ──────→ Tavily 병렬 검색 12쿼리
       │                           (리스크 쿼리 우선 배치)
       │
       ├──[3]──→ Retrieval Judge → 관련성 점수 ≥ 0.6 검증
       │              │ 실패 → [1] 재검색 (revised_query)
       │
       ├──[4]──→ Analysis Agent → TRL + 경쟁사 JSON (Pydantic)
       │
       ├──[5]──→ TRL Judge ──────→ 정규식 근거 검증 (LLM 없음)
       │              │ 실패 → [4] 재분석
       │
       ├──[6]──→ Draft Agent ────→ JSON → 마크다운 보고서
       │
       └──[7]──→ Formatting Node → 마크다운 정제 + PDF 출력
                                    outputs/report_*.{md,pdf}
```
